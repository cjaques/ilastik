...
class CountingArtetaGui(LabelingGui):
    ...
    def stopAndCleanUp(self):
        # Base class
        super(CountingArtetaGui, self).stopAndCleanUp()
    ...
    def __init__(self, parentApplet, topLevelOperatorView):
        ...
        # Base class init
        super(CountingArtetaGui, self).__init__(parentApplet, labelSlots, topLevelOperatorView, labelingDrawerUiPath )
    ...
    def initCounting(self):
        ...
        self.labelingDrawerUi.TrainButton.clicked.connect(self.trainClassifier)
    ...
    def _registerOperatorsToGuiCallbacks(self):
        ...
        def _setoption(option):
            ...
            print 'Selected option is : ', gui.SVROptions, ' at index : ',index
    ...
    def _setUIParameters(self):
        ...
        self.labelingDrawerUi.MaxDepthBox.setRange(0,1000)
        ...
        if hasattr(cache._value, "__iter__") and len(self.op.classifier_cache._value) > 0:
            ...
            _ind = -1 #self.labelingDrawerUi.AlgorithmList.findText(self.op.opTrain.SelectedOption.value)
    ...
    def setupLayers(self):
        ...
        layers = super(CountingArtetaGui, self).setupLayers()
        ...
        slots = { 'Prediction' : (self.op.Density, 1.0), 
        ...
        for name, (slot, opacity) in slots.items():
            if slot.ready():
                ...
                layer = ColortableLayer(LazyflowSource(slot), colorTable=countingColorTable, normalize=(0,self.upperBound))
                layer.name = name
...
#        self.labelingDrawerUi.savePredictionsButton.setEnabled(not checked)
    ...
    def updateAllLayers(self, slot=None):
        super(CountingArtetaGui, self).updateAllLayers()
    ...
    def _onLabelRemoved(self, parent, start, end):
        super(CountingArtetaGui, self)._onLabelRemoved(parent, start, end)
    ...
    def getNextLabelName(self):
        return self._getNext(self.topLevelOperatorView.LabelNames,
                             super(CountingArtetaGui, self).getNextLabelName)

    def getNextLabelColor(self):
        return self._getNext(
            self.topLevelOperatorView.LabelColors,
            super(CountingArtetaGui, self).getNextLabelColor,
            lambda x: QColor(*x)
        )
    ...
    def getNextPmapColor(self):
        return self._getNext(
            ...
            super(CountingArtetaGui, self).getNextPmapColor,
            lambda x: QColor(*x)
        )

    def onLabelNameChanged(self):
        self._onLabelChanged(super(CountingArtetaGui, self).onLabelNameChanged,
                             lambda l: l.name,
                             self.topLevelOperatorView.LabelNames)

    def onLabelColorChanged(self):
        self._onLabelChanged(super(CountingArtetaGui, self).onLabelColorChanged,
    ...
    def onPmapColorChanged(self):
        self._onLabelChanged(super(CountingArtetaGui, self).onPmapColorChanged,
...
#         self._labelControlUi.brushSizeCaption.setEnabled(False)
    ...
    def _initLabelUic(self, drawerUiPath):
        super(CountingArtetaGui, self)._initLabelUic(drawerUiPath)
